...
public class GameController {
    ...
    public void onMoveUpdate(TileDTO tile, String currPlayer) {
        ...
        QTileGUI source = (QTileGUI) tiles.getChildren().getFirst();
    ...
    public void onCurrPlayerUpdate(String nickname) {
        ...
        txtCurrPlayer.setText("turno di: " + nickname.toLowerCase());
        String color = opponentColors.getOrDefault(nickname, "#ffffff");
        txtCurrPlayer.setFill(Color.web(color));
    ...
    public void onReturnToQueue(TileDTO t, PlayerStatsDTO s) {
        ...
        QTileGUI target = (QTileGUI) tiles.getChildren().getFirst();
    ...
    public void onOpponentDraw(CardDTO card, String nickname) {
        ...
        Text notif = new Text(nickname + " ha pescato: " + typeName);
        notif.setFill(Color.web(color));
        notif.setStyle("-fx-font-size: 20; -fx-font-weight: bold;");

        opponents.getChildren().addFirst(notif);

        PauseTransition delay = new PauseTransition(Duration.millis(2500));
        delay.setOnFinished(event -> opponents.getChildren().remove(notif));
        delay.play();
    ...
    public void notifySkip(String nickname) {
        ...
        Text notif = new Text(nickname + " ha saltato il turno.\n");
        notif.setFill(Color.web(color));
        notif.setStyle("-fx-font-size: 20; -fx-font-weight: bold;");
        ...
        delay.setOnFinished(event -> {
            opponents.getChildren().remove(notif);
        });
        delay.play();
    }
    ...
    private void updateTotemPositions() {
        ...
        QTileGUI qTile = (QTileGUI) tiles.getChildren().getFirst();
        qTile.clearPawns();
    ...
    private void updateCardRows() {
        ...
        for (CardDTO dto : viewGUI.getModel().getUpperList()) {
            if(dto.getType().isCharacter()) {
                ...
                upperRowChar.getChildren().add(cardGUI);
                cardGUI.setViewOrder(upperRowChar.getChildren().size() - 1);
                cardGUI.showSliding(upperRowChar, upperRowBuildsEv);
            } else {
                upperRowBuildsEv.getChildren().add(createDrawableCard(dto.getId()));
            }
        }
        ...
        for (CardDTO dto : viewGUI.getModel().getLowerList()) {
            if (dto.getType().isCharacter()) {
                CardGUI cardGUI = createDrawableCard(dto.getId());
                lowerRowChar.getChildren().add(cardGUI);
                cardGUI.setViewOrder(lowerRowChar.getChildren().size() - 1);
                cardGUI.showSliding(lowerRowChar, lowerRowBuildsEv);
            } else {
                lowerRowBuildsEv.getChildren().add(createDrawableCard(dto.getId()));
            }
        }
        refreshDrawableHighlight();
    }
    ...
    private void initCards(List<CardDTO> cardsUp, List<CardDTO> cardsDown) {
        ...
        for (CardDTO dto : cardsUp) {
            if(dto.getType().isCharacter()) {
                ...
                CardGUI cardGUI = createDrawableCard(dto.getId());
                upperRowBuildsEv.getChildren().add(cardGUI);
            }
        }

        lowerRowChar.getChildren().clear();
        lowerRowBuildsEv.getChildren().clear();
        for (CardDTO dto : cardsDown) {
            if (dto.getType().isCharacter()) {
                CardGUI cardGUI = createDrawableCard(dto.getId());
                lowerRowChar.getChildren().add(cardGUI);
                cardGUI.setViewOrder(lowerRowChar.getChildren().size() - 1);
                ...
                CardGUI cardGUI = createDrawableCard(dto.getId());
                lowerRowBuildsEv.getChildren().add(cardGUI);
            }
        }
        refreshDrawableHighlight();
    }
    ...
    private void initBoard(int numPlayers, List<TileDTO> tileList) {
        ...
        tiles.getChildren().add(new QTileGUI(numPlayers));
    ...
    private void initQueue(List<TileDTO> tileList) {
        assertFxThread();
        QTileGUI qTile = (QTileGUI) tiles.getChildren().get(0);
    ...
    private void addOpponentRows(String name, PlayerStatsDTO stats) {
        ...
        nameButton.setStyle("-fx-text-fill: " + color + "; -fx-font-weight: bold;");
        nameButton.setOnAction(e -> viewVillage(name));
        nameButton.setBackground(Background.EMPTY);
        nameButton.setBorder(Border.EMPTY);

        Text statsText = buildStatsText(stats);
        opponentStatsNodes.put(name, statsText);

        opponents.getChildren().addAll(nameButton, statsText);
    }
