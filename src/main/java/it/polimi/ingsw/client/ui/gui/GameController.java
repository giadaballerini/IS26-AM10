...
public class GameController {
    ...
    /** Lower row of the character card deck area, displaying the face-up character cards available for drawing. */
    @FXML
    private HBox lowerRowChar;

    /** Upper row of the character card deck area, displaying the face-down character cards. */
    @FXML
    private HBox upperRowChar;

    /** Lower row of the building and event card deck area, displaying the face-up building/event cards available for drawing. */
    @FXML
    private HBox lowerRowBuildsEv;

    /** Upper row of the building and event card deck area, displaying the face-down building/event cards. */
    @FXML
    private HBox upperRowBuildsEv;

    /** Horizontal container holding the active flag icons for the current round (e.g. hunt, protection flags). */
    @FXML
    private HBox flagBox;

    /** Horizontal container rendering the local player's village card slots. */
    @FXML
    private HBox myVillage;

    /** Vertical container listing the opponents' summary panels in the side bar. */
    @FXML
    private VBox opponents;

    /** Text node displaying the local player's current building discount value. */
    @FXML
    private Text txtBuildDisc;

    /** Text node displaying the local player's current feast discount value. */
    @FXML
    private Text txtFeastDisc;

    /** Text node displaying the local player's nickname. */
    @FXML
    private Text txtNickname;

    /** Text node displaying the local player's current progress points (PP) count. */
    @FXML
    private Text txtPP;

    /** Text node displaying the local player's current food resource count. */
    @FXML
    private Text txtFood;

    /** Text node displaying the local player's current star count. */
    @FXML
    private Text txtStars;

    /** TextFlow container for the "current player" label, allowing mixed-style text segments. */
    @FXML
    private TextFlow txtCurrPlayer;

    /** Text node displaying the nickname of the player whose turn it currently is. */
    @FXML
    private Text txtCurrPlayerNick;

    /** Text node displaying the current game era (age) number. */
    @FXML
    private Text txtAge;

    /** Text node displaying the current round number within the era. */
    @FXML
    private Text txtRound;

    /** Text node displaying the current game phase (e.g. Setup, Move, Draw, End-of-round). */
    @FXML
    private Text txtPhase;

    /** Horizontal container holding all {@link TileGUI} nodes that make up the game board. */
    @FXML
    private HBox tiles;

    /** Icon image for the progress points (PP) stat label. */
    @FXML
    private ImageView imgPP;

    /** Icon image for the food resource stat label. */
    @FXML
    private ImageView imgFood;

    /** Icon image for the stars stat label. */
    @FXML
    private ImageView imgStars;

    /** Icon image for the building discount stat label. */
    @FXML
    private ImageView imgBuildDisc;

    /** Icon image for the feast discount stat label. */
    @FXML
    private ImageView imgFeastDisc;

    /** Full-screen overlay pane used to display event banners and era-transition curtain animations. */
    @FXML
    private StackPane globalOverlay;

    /** Horizontal container placed inside {@link #globalOverlay} that holds the overlay's inner content (e.g. event cards layout). */
    @FXML
    private HBox overlayContent;

    /** Button that lets the local player skip the optional draw phase when no draw is desired or available. */
    @FXML
    private Button skipButton;

    /** Clickable image that opens the in-game pause/settings menu. */
    @FXML
    private ImageView menuBtn;

    /** Button that lets the local player quit the current match and return to the main menu. */
    @FXML
    Button quitBtn;

    /** Text node used to define where the flags are displayed in the game board. */
    @FXML
    Text flagTxt;

    /** Vertical container used to stack transient toast-style notifications (e.g. error messages, turn alerts). */
    @FXML
     ...
     */
    private Map<String, PlayerStatsDTO> statsBeforeEvent = new ConcurrentHashMap<>();

    /**
     * Queue of event banners waiting to be displayed.
     * Necessary because the server may send two consecutive {@code EventMessage}s
     * in the same round (round events and end-game events); the second must not be
    ...
    public void initialize(){
        ...
        createTooltip(imgPP, "Punti Prestigio", "I tuoi punti totali accumulati fino ad ora, vince chi ne ha di più!");
        createTooltip(imgFood, "Riserve di Cibo", "La quantità di cibo disponibile per sfamare la tua tribù.");
        createTooltip(imgStars, "Stelle", "La quantità di stelle che i tuoi sciamani detengono magicamente.");
        createTooltip(imgBuildDisc, "Sconto Costruzione", "Riduce il costo richiesto per acquistare edifici.");
        createTooltip(imgFeastDisc, "Sconto Sostentamento", "Sconto applicato durante l'evento di sostentamento.");
    }

    /**
     * FXML handler invoked when the player clicks the {@link #menuBtn} image.
     *
     * <p>Convenience overload that delegates to {@link #onMenuBtnClick(MouseEvent, Node)}
     * with {@code null} as the {@code fallback} node, so that any confirmation
     * dialog's "Back" button returns to the pause menu itself rather than to
     * a previous overlay.</p>
     *
     * @param event the mouse-click event fired by the FXML binding
     */
    @FXML
     ...
     *
     * <p>The menu exposes three actions: close the application entirely, abandon the
     * current match and return to the main menu (triggers {@link it.polimi.ingsw.client.Client#quit()}),
    ...
    private void onMenuBtnClick(MouseEvent event, Node fallback) {
        ...
        exitBtn.getStyleClass().add("mesos-button");
        exitBtn.setPrefWidth(250);
        exitBtn.setPadding(Insets.EMPTY);
        exitBtn.setOnAction(e -> showConfirm(
                "Vuoi davvero uscire?",
                "L'applicazione verrà chiusa.",
                Platform::exit
        ));

        Button quitGameButton = new Button("Abbandona partita");
        quitGameButton.getStyleClass().add("mesos-button");
        quitGameButton.setPrefWidth(250);
        quitGameButton.setPadding(Insets.EMPTY);
        quitGameButton.setOnAction(e -> showConfirm(
        ...
        cancelBtn.getStyleClass().add("mesos-button");
        cancelBtn.setPrefWidth(250);
        cancelBtn.setOnAction(e -> hideOverlay(fallback));

        VBox menu = new VBox(14, titleLabel, exitBtn, quitGameButton, cancelBtn);
    ...
    private void showConfirm(String title, String caption, Runnable onConfirm, Node fallback) {
        ...
        confirmBtn.getStyleClass().add("mesos-button");
        ...
        backBtn.getStyleClass().add("mesos-button");
        backBtn.setPrefWidth(200);
        backBtn.setOnAction(e -> onMenuBtnClick(null, fallback));
     ...
     * @param desc longer description shown below the title
     */
    private void createTooltip(ImageView target, String title, String desc) {
    ...
    public void refreshBoard() {
        ...
        if (firstCall) {
            villagePanel.snapCollapsed();
            statsPanel.snapCollapsed();
        }
    ...
    public void notifySkip(String nickname) {
        ...
        showNotification(nickname + " ha saltato il turno.", color);
    }
    ...
    /**
     * Populates and reveals the global leaderboard table inside the
     * {@link LeaderboardController} that was loaded during
     * {@link #onGameEnding(List, int, int)}.
     *
     * <p>Called by {@link ViewGUI#showLeaderboard(Map)} on the FX Application
     * Thread after the server responds to a global-ranking request. The map is
     * forwarded to the leaderboard controller, which sorts the entries and
     * renders them in the persistent-ranking table view.</p>
     *
     * <p>Must be called on the FX Application Thread; enforced by
     * {@link #assertFxThread()}.</p>
     *
     * @param ranks a map from each {@link PlayerDTO} to their position in the
     *              persistent global leaderboard (1-based, ties share a position)
     */
     ...
     * {@link #refreshBoard()} is called repeatedly.</p>
     *
     * <p>Must be called on the FX Application Thread.</p>
     */
    ...
    /**
     * Synchronizes all dynamic GUI elements with the current state of the
     * {@link it.polimi.ingsw.client.VirtualModel}.
     *
    ...
    private void syncState() {
        ...
        flagBox.getChildren().add(flagTxt);
        initFlags();
     ...
     * {@code buildEvRow}. Character cards also have sliding behavior enabled.
     *
    ...
    private void updateInfos(int age, int turn, String nickname, GamePhaseEnum phaseState) {
        if (nickname == null || nickname.isEmpty()) {
            txtAge.setText("in ");
            txtRound.setText("attesa di ");
            txtPhase.setText("giocatori!");
            txtCurrPlayer.setVisible(false);
            txtCurrPlayer.setManaged(false);
        } else {
            txtAge.setText("era: " + age);
    ...
    private void handleDrawClick(CardGUI card) {
        ...
        drawTask.setOnSucceeded(event -> card.setDisable(false));
    ...
    private void initBoard(int numPlayers, List<TileDTO> tileList) {
        ...
        for (int i = 0; i < tileList.size(); i++) {
            ...
            TileGUI newTile = new TileGUI(tile.getId());
            final int index = i;
            newTile.setOnMouseClicked(event -> handleMoveClick(newTile, index));
            tileMap.put(tile.getId(), newTile);
    ...
    /**
     * Updates the statistics display for all players.
     *
     ...
     * opponent's statistics row is updated in place via
    ...
    public void updateStats(List<PlayerStatsDTO> playerStats) {
        ...
        if (!eventBannerShowing && statsBeforeEvent.isEmpty() && isEventPhase) {
            statsBeforeEvent = viewGUI.getModel().getPlayerStats().stream()
                    .collect(Collectors.toMap(PlayerStatsDTO::getNickname, s -> s));
        }
        String myNickname = viewGUI.getNickname();
    ...
    private void addFlagIcon(String flag) {
        ...
        createTooltip(icon, flag, FlagRegistry.getDescription(flag));
    }
    /**
     ...
     * optional (as indicated by {@link ActionsDTO#isSkippable()}).</p>
     *
     * <p>Must be called on the FX Application Thread.</p>
     */
    private void refreshDrawableHighlight() {
        ...
        boolean canSkip = (phase.equals(GamePhaseEnum.DRAW_PHASE) || phase.equals(GamePhaseEnum.OPTIONAL_DRAW_PHASE))
                ...
                && viewGUI.getModel().getToDoActions().isSkippable();
    ...
    private void hideBanner(){
        ...
        javafx.scene.Node banner = overlayContent.getChildren().getFirst();
    ...
    private static String formatStats(PlayerStatsDTO stats) {
        return "cibo: " + stats.getnFood()
                + "  pp: " + stats.getPPs()
                + "  stelle: " + stats.getnStars()
    ...
    /**
     * Immutable value object that groups the three JavaFX nodes making up
     * a single opponent's village overlay.
     *
     * <p>One instance is created per opponent during {@link #initOpponentOverlays}
     * and stored in {@link #opponentOverlays}, keyed by the opponent's nickname.
     * When the local player clicks the "+" button next to an opponent's name,
     * {@link #viewVillage(String)} passes {@link #root} to {@link #showOverlay}
     * so the opponent's card collection is rendered inside {@link #globalOverlay}.
     * On every {@link #updateVillages()} call, {@link #cols} is updated with the
     * opponent's current cards.</p>
     */
    private static class VillageOverlay {

        /**
         * The root {@link VBox} injected into {@link #globalOverlay} when the
         * overlay is shown; contains the opponent's title, {@link #content},
         * and the close button.
         */
        final VBox root;

        /**
         * Horizontal container inside {@link #root} that holds all per-type
         * card columns ({@link #cols}) side by side.
         */
        final HBox content;

        /**
         * Maps each {@link CardTypeEnum} to the {@link VBox} column that stacks
         * the opponent's cards of that type. A column is hidden ({@code visible = false})
         * until at least one card of the corresponding type is present.
         */
        final Map<CardTypeEnum, VBox> cols;

        /**
         * Constructs a fully assembled {@code VillageOverlay}.
         *
         * @param root    the root container to be shown inside {@link #globalOverlay}
         * @param content the horizontal container holding the card-type columns
         * @param cols    the per-{@link CardTypeEnum} column map
         */
