...
public class LeaderboardController {
    ...
    /** Button that returns from the global leaderboard view to the match ranking view. Hidden until the global table is shown. */
    @FXML private Button backToMatchBtn;
    ...
    public void initialize() {
        ...
        backToMatchBtn.setVisible(false);
        backToMatchBtn.setManaged(false);
    }
     ...
     * layout clean, while {@link #backToMatchBtn} is revealed so the player
     * can return to the match ranking via {@link #onBackToMatchRanking()}.</p>
     *
     * <p>Must be called on the FX Application Thread.</p>
     *
    ...
    public void showGlobalRanking(Map<String, Integer> ranks) {
        ...
        backToMatchBtn.setVisible(true);
        backToMatchBtn.setManaged(true);
    }



    /**
     * Handles the "Indietro" button click: hides the global leaderboard
     * view and brings back the end-of-match ranking view, restoring the
     * "Classifica globale" button so the global ranking can be reopened.
     *
     * <p>Must be called on the FX Application Thread (it is triggered
     * directly by a button click on that thread).</p>
     */
    @FXML
    public void onBackToMatchRanking() {
        assertFxThread();

        globalTable.setVisible(false);
        globalTable.setManaged(false);
        rankingTable.setVisible(true);
        rankingTable.setManaged(true);
        backToMatchBtn.setVisible(false);
        backToMatchBtn.setManaged(false);
        globalLeaderboardBtn.setVisible(true);
        globalLeaderboardBtn.setManaged(true);
    }
    /**
     * Creates a new {@code LeaderboardController} instance.
