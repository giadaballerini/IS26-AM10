...
public class TileGUI extends PopupPane {
    ...
    /** The numeric ID of this tile, used to retrieve its image and metadata. */
    ...
    /**
     * Pixel bounds of the arrow indicator area within the rendered tile image.
     * {@code null} for tiles that have no draw actions (e.g. {@code Tile_0}).
     */
    ...
    public TileGUI(int tileId) {
        ...
        double ratio = TileImagesLoader.getInstance().getFront(this.tileId).getHeight()
                / TileImagesLoader.getInstance().getFront(this.tileId).getWidth();
