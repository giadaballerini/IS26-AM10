...
public class TUICompleter implements Completer {
     ...
     * {@code info}, {@code quit}, {@code pgup}, {@code pgdn}) are suggested only during an
     * active match, further filtered by the current phase and the available draw actions.
     *
    ...
    private void completeCommands(List<Candidate> candidates) {
        ...
        if (phase == null || phase == GamePhaseEnum.NONE) {
            ...
            candidates.add(new Candidate("pgup"));
            candidates.add(new Candidate("pgdn"));
