...
public class ViewGUI implements UserInterface {
    ...
    /** The client instance that owns this GUI. */
    private Client client;

    /** The model that drives the game state. */
    ...
    /**
     * Flag set to {@code true} when the client is reconnecting to an ongoing
     * match. The game state is not updated until the server has restored the
     * match state.
     */
    private volatile boolean boardSyncedAfterReconnect = false;
    ...
    public void setGameController(GameController gameController) {
        ...
        if (gameController != null) {
            if (pendingBoardRefresh) {
                ...
                boardSyncedAfterReconnect = true;
            }
        }
    }
     ...
     *
     * <p>If {@link #gameController} is not yet available, the request is deferred via {@link #pendingBoardRefresh}
     ...
     * after occupying a tile and receiving their statistics update.
     *
     ...
     *
     * <p>Typically called when the server rejects an action.</p>
     *
    ...
    public void onServerCrash() {
        Platform.runLater(() -> {
            ...
            MainMenuController mc = menuController;
            if (mc != null)
                mc.returnToMenu("Connessione al server persa. Riavviare e riconnettersi.");
        });
    }
    ...
    public void reconnect(int matchId) {
        boardSyncedAfterReconnect = false;
        Platform.runLater(() -> {
            MainMenuController mc = menuController;
    ...
    public void reconnectWaiting() {
        if (boardSyncedAfterReconnect) return;
        boardSyncedAfterReconnect = true;
        GameController gc = gameController;
