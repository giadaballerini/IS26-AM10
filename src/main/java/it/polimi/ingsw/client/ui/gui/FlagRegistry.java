...
public class FlagRegistry {
    ...
    /** Logger used to report failures during the {@code clientFlags.json} loading in {@link #loadFlags()}. */
    private static final Logger LOG = Logger.getLogger(FlagRegistry.class.getName());
    ...
    private static void loadFlags() {
        ...
        try (InputStream is = FlagRegistry.class.getResourceAsStream("/json/clientFlags.json")) {
            ...
            List<FlagData> descriptions = mapper.readValue(is, new TypeReference<>() {
            });
            ...
            LOG.log(Level.SEVERE, "Fallimento nel caricamento clientFlags.json", e);
        }
    }
