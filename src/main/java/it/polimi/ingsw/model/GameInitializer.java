...
public class GameInitializer {
    ...
    public List<Card> initBuildingDeck(){
        ObjectMapper mapper = new ObjectMapper();
        mapper.enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY);
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    ...
    public List<Card> initUpperList(List<Card> deck,List<Card> buildings, int numPlayers) {
        ...
        for(int i = 0; i < buildingsToAdd && !buildings.isEmpty(); i++) {
    ...
    public List<Card> initLowerList(List<Card> deck,List<Card> upperList, int numPlayers) {
        ...
        while(lowerList.size() < size && !deck.isEmpty()) {
            ...
            if(card.getType() == CardTypeEnum.HUNT || card.getType() == CardTypeEnum.RITUAL || card.getType() == CardTypeEnum.FEAST || card.getType() == CardTypeEnum.STONE_PAINTING)
                upperList.add(card);
            else
                lowerList.add(card);
        }
